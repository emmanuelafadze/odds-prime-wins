...
function Pricing() {
  ...
  const buy = async (key: keyof typeof PRICING) => {
    ...
    toast.message(`Opening Paystack checkout for GH₵${ghsAmount.toFixed(2)}`);
    ...
    <SiteLayout>
      <section className="container mx-auto px-4 py-20">
        <div className="text-center">
          ...
          <p className="mt-3 text-muted-foreground">Pay-per-tip in GH₵ via Paystack. Instant access. Pay once, win smart.</p>
        </div>
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {Object.entries(PRICING).map(([k, p]) => (
            <Card key={k} className={`flex flex-col p-6 ${k === "premium" ? "border-primary bg-gradient-to-b from-primary/10 to-transparent shadow-[var(--shadow-elegant)]" : ""}`}>
              ...
              <div className="mt-3 text-4xl font-extrabold">GH₵{p.price}</div>
              ...
              <Button className="mt-6" onClick={() => buy(k as keyof typeof PRICING)} disabled={loadingKeys.has(k as keyof typeof PRICING)}>
                {loadingKeys.has(k as keyof typeof PRICING) ? "Loading..." : "Buy with Paystack"}
              </Button>
            </Card>
          ))}
        </div>
