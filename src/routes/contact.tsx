...
export const Route = createFileRoute("/contact")({
  head: () => ({ meta: [{ title: "Contact — ODDSPrime" }, { name: "description", content: "Reach the ODDSPrime team by phone or on Signal." }] }),
  component: () => (
    <SiteLayout>
      <section className="container mx-auto max-w-4xl px-4 py-20">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          <Card className="p-6"><Phone className="h-6 w-6 text-primary" /><h3 className="mt-3 font-semibold">Call / Signal</h3><a href={`tel:${CONTACT_PHONE.replace(/\s/g,'')}`} className="mt-1 text-sm text-muted-foreground hover:text-primary">{CONTACT_PHONE}</a><p className="mt-2 text-xs text-muted-foreground">This number is available on Signal.</p></Card>
